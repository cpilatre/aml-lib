...
impl std::fmt::Display for AmlError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        let text = match *self {
            AmlError::UnimplementedVersion => String::from("You have tried to parse an unimplemented version of SMS AML"),
            AmlError::InvalidBase64 => String::from("You have tried to parse an corrumpted base64 SMS data")
        };
        write!(f, "AmlError: {}", text)
    }
}